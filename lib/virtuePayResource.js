...
VirtuePayResource.prototype = {
  ...
  _timeoutHandler: function (timeout, req, callback) {
    ...
    return function () {
      ...
      callback.call(
        self,
        new Error.VirtuePayConnectionError({
        ...
        try {
          ...
          if (response.error) {
            ...
            if (res.statusCode === 401) {
              err = new Error.VirtuePayAuthenticationError(response.error);
            } else if (res.statusCode === 403) {
              err = new Error.VirtuePayRateLimitError(response.error);
            } else {
              err = Error.VirtuePayError.generate(response.error);
            }
            return callback.call(self, err, null);
          }
        } catch (e) {
          err = new Error.VirtuePayAPIError({
  ...
  _request: function (method, path, data, auth, callback) {
    ...
    var apiVersion = this._virtuepay.getApiField('version');
    var headers = {
      'Authorization': auth ?
        'Bearer ' + auth :
        this._virtuepay.getApiField('auth'),
      'Accept': 'application/json',
      'Content-Type': contentType + '; charset=UTF-8',
      'User-Agent': 'VirtuePay/v1 NodeBindings/'
        + this._virtuepay.getConstant('PACKAGE_VERSION')
    };

    var requestTime = Date.parse(new Date()) / 1000;
    if (this._virtuepay.getPrivateKey()) {
      headers['X-Signature'] = utils.generateSign(
        requestData + path + requestTime,
        this._virtuepay.getPrivateKey()
      );
    }
    ...
    headers = _.assign(headers, this._virtuepay.getParsedHeaders());

    this._virtuepay.getClientUserAgent(function (cua) {
      headers['VirtuePay-Client-User-Agent'] = cua;
      makeRequest();
    });
    ...
    function makeRequest() {
      var timeout = self._virtuepay.getApiField('timeout');
      var isInsecureConnection = self._virtuepay.getApiField('protocol') == 'http';

      var req = (
        isInsecureConnection ? http : https
      ).request({
        host: self._virtuepay.getApiField('host'),
        port: self._virtuepay.getApiField('port'),
        path: path,
        method: method,
        headers: headers,
